...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val set = left.filter(p) union right.filter(p)
    if (p(elem)) set incl elem
    else set
  }
  ...
  def mostRetweeted: Tweet = {
    val set = left.filter(t => t.retweets > elem.retweets) union right.filter(t => t.retweets > elem.retweets)
    if (set.isEmpty) elem
    else set.mostRetweeted
  }
...
object Main extends App {
  ...
  // println((GoogleVsApple.googleTweets union GoogleVsApple.appleTweets).mostRetweeted.retweets)
